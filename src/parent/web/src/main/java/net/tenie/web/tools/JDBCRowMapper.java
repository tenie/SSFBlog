...
//
//import net.tenie.base.config.SpringContextHolder;
...
public class JDBCRowMapper implements RowMapper{
	...
	public static void main(String[] args) {
		 JdbcTemplate temp= ApplicationContextHelper.getBeanByType(JdbcTemplate.class);
		 JDBCRowMapper mapper = new JDBCRowMapper();
		 List arg = new ArrayList<>();
		 arg.add("14b05780-deba-11e6-a53a-4ccc6a5cbd3c");
		 temp.query("SELECT * FROM f4_task_lock where LOCK_ID=?",arg.toArray(), mapper);
		 List<Map> list = (ArrayList<Map>) mapper.getList();
		 for(Map m:list){
			 String i =  (String) m.get("LOCK_ID");
