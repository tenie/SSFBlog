...
// 首页
// 主页标题信息渲染
ssfblog.initIndex=function(datas,callback){  
	setTimeout(function(){ 
	...
	$.get("/index_contentTitle.html",function(htmldata){  
		var f = ""
		...
		var time = pageTitleContainer.find(".post_time")	 
			 ...
			 $.get("/loginPage.html",function(htmldata){  
					...
					$("#submitSignIndata").click(function(){
						...
						$.post("/login",$("#signInForm").serialize(),function(data){  
							if(!data.error){ // 登入成功 : 关闭界面,
												// 提示成功,在浏览器session中保存登入过,
												// 隐藏登入按钮,现实登出按钮
...
ssfblog.toastr=function(method,msg){
	 new Noty({
	      text: msg,
	      type:method  ,  // var types = ['alert', 'warning', 'success',
							// 'information', 'error']
	      theme: 'metroui',
	      layout: 'topRight',
	      timeout: 2000,
...
// 分页记录, 参数:limit, offset, 上一页下一页按钮, 总行数(总行数在初始化的时候不设置,
// 但获取到总行数后,再设置,这样下一页才会知道到哪一页停止)
...
ssfblog.publishPage_html=function(){ 
	...
	$('input[name="publishPagecheckbox"]').bootstrapSwitch({
				...
				offColor:"warning",
				/*
				 * onInit:function(event, state) { this.state=false },
				 */
			    onSwitchChange:function(event, state) {
